...
\begin{remark}
Note that the choice of kernel \(K\) and bandwidth \(h\) is not unique.
For a given kernel shape, different authors use different normalisations.
For example, we could replace our triangular kernel~\(K\)
from equation~\eqref{eq:triangular} with \(\tilde K(x) = K(x/\sqrt{6})\),
which has variance~1. If we simultaneously replace \(h\) with \(\tilde h = h/\sqrt{6}\),
then the rescaled kernel remains unchanged: \(\tilde K_{\tilde h} = K_h\).
\end{remark}
...
##  KDE1 835.129 884.9030 952.8551 902.369 923.6275 5157.636  1000
##  KDE2 110.413 156.0665 234.9346 160.638 169.0840 5590.883  1000
##  KDE3  98.810 131.6920 165.6060 135.874 142.3315 4323.983  1000
\end{verbatim}
...
## Unit: microseconds
##     expr    min      lq     mean   median      uq      max neval
##  density 92.209 99.3225 112.8166 102.3975 105.985 3175.532  1000
\end{verbatim}

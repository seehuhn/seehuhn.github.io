...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:snow-dens-col}This figure illustrates the effect of the bandwidth parameter on the kernel density estimate. Small bandwidths give a more detailed estimate, but also show more noise. Large bandwidths give a smoother estimate, but may miss local structure of the density.}]{MATH5714M_files/figure-latex/snow-dens-col-1.pdf}}
\caption{\label{fig:snow-dens-col}This figure illustrates the effect of the bandwidth parameter on the kernel density estimate. Small bandwidths give a more detailed estimate, but also show more noise. Large bandwidths give a smoother estimate, but may miss local structure of the density.}
\end{figure}
...
##  KDE1 824.305 843.7595 880.2882 849.9505 859.2985 4102.952  1000
##  KDE2 112.340 113.7750 144.7461 114.3900 115.3535 1901.129  1000
##  KDE3 100.573 102.2540 129.8623 102.7870 103.7300 3241.460  1000
\end{verbatim}
...
## Unit: microseconds
##     expr    min     lq     mean median      uq      max neval
##  density 90.487 92.783 111.0257 99.835 106.354 5846.272  1000
\end{verbatim}
...
\subsection{The Roughness of Kernels}\label{the-roughness-of-kernels}

In addition to the moments of a kernel, we will need one more kernel property
for our analysis.
...
The roughness measures how concentrated the kernel is: a kernel with higher
roughness has more mass concentrated in a smaller region, while a kernel with
lower roughness is more spread out.

Similar to the moments, the roughness of the rescaled kernel \(K_h\) can be
expressed in terms of the roughness of \(K\).

\begin{lemma}
\protect\hypertarget{lem:Kh-rough}{}\label{lem:Kh-rough}Let \(K\) be a kernel and \(h > 0\). Then
\begin{equation*}
  R(K_h)
  = \frac{1}{h} R(K).
\end{equation*}
...
\begin{align*}
  R(K_h)
  &= \int_{-\infty}^\infty K_h(x)^2 \,dx \\
  &= \int_{-\infty}^\infty \Bigl( \frac{1}{h} K\Bigl(\frac{x}{h}\Bigr) \Bigr)^2 \,dx \\
  &= \int_{-\infty}^\infty \frac{1}{h^2} K\Bigl(\frac{x}{h}\Bigr)^2 \,dx.
\end{align*}
Using the substitution \(y = x/h\) we find
\begin{align*}
  R(K_h)
  &= \int_{-\infty}^\infty \frac{1}{h^2} K(y)^2 \, h \,dy \\
  &= \frac{1}{h} \int_{-\infty}^\infty K(y)^2 \,dy \\
  &= \frac{1}{h} R(K).
\end{align*}
This completes the proof.
\end{proof}
...
\paragraph{\texorpdfstring{Approximating \(\mathbb{E}\bigl( K_h(x - X_1)^2 \bigr)\)}{Approximating \textbackslash mathbb\{E\}\textbackslash bigl( K\_h(x - X\_1)\^{}2 \textbackslash bigr)}}\label{approximating-mathbbebigl-k_hx---x_12-bigr}
...
antisymmetric function. Using the definition of the roughness of a kernel,
\(R(K) = \int K(x)^2 \,dx\), this gives the result
\begin{equation}
  \mathbb{E}\bigl( K_h(x - X_1)^2 \bigr)
...
Triangular & \includegraphics[width=4cm,height=\textheight,keepaspectratio,alt={triangular kernel}]{MATH5714M_files/figure-html/kTriangular-1.pdf} & \(\displaystyle\frac16\) & \(\displaystyle\frac23\) & \(0.07407\) \\
...
See \texttt{help("bw.nrd0")} for R's built-in bandwidth selectors. For comparison to our result,
...
All of these values seem close the value we obtained manually. Using
...
In this setup, a \textbf{kernel} is a function \(K\colon \mathbb{R}^p\to \mathbb{R}\) (generalizing Definition \ref{def:kernel}) such that
...
In the previous sections we have used kernel methods to estimate
probability densities from observed data. In the following sections
we will see a different application of kernel methods: using them for
regression, where we estimate a function \(m\) from noisy observations.
...
\tightlist
\item
  The Epanechnikov kernel minimises \(R(K)^2 \mu_2(K)\) and gives the smallest
  integrated error, though differences between common kernels are small.
\item
  Bandwidth selection uses plug-in rules that estimate \(R(f'')\) from the data,
  often by assuming a normal reference distribution.
\item
  R provides built-in bandwidth selectors such as \texttt{bw.nrd0()}, \texttt{bw.nrd()}, and
  \texttt{bw.SJ()} for practical use.
\item
  Kernel density estimation extends to higher dimensions \(\mathbb{R}^p\) using product
  kernels, with bandwidth parameters that can differ across coordinates.
\end{itemize}
...
The problem of finding \(m\) using kernel functions is called \textbf{kernel smoothing}
...
limit of \(n\to \infty\), \emph{i.e.}~for the case when we have a large dataset
...
will be very similar to the arguments used in Section~\ref{X02-error-var}
on the variance of kernel density estimates.
...
\begin{align*}
  ...
  &= \mathbb{E}\Bigl( K_h(x - X) (m(X) + s(X)\eta) \Bigr).
\end{align*}
We use integrals to average over the randomness in \(X\) and \(\eta\),
denoting the density of \(\eta\) by~\(\varphi\):
\begin{align*}
  \mathbb{E}\bigl( \hat r_h(x) \bigr)
  &= \int \int K_h(x - \xi) \bigl( m(\xi) + s(\xi) e \bigr) \, \varphi(e) \, de \, f(\xi) \, d\xi \\
  &= \int K_h(x - \xi) \Bigl( m(\xi) + s(\xi) \int e \, \varphi(e) \, de \Bigr) \, f(\xi) \, d\xi \\
...
\begin{proof}
The first statement is proved using substitution and Taylor expansion of \(r\)
...
\begin{align*}
  ...
  &= \frac{r(x)}{f(x)}
    ...
    - \frac12 \frac{\mu_2(K) r(x) f''(x)}{f(x)^2} h^2
    + o(h^2)
\end{align*}
...
\begin{align*}
  ...
  &= m(x)
    + \frac12 \frac{\mu_2(K) r''(x)}{f(x)} h^2
    - \frac12 \frac{\mu_2(K) m(x) f''(x)}{f(x)} h^2
    + o(h^2) \\
  &= m(x)
...
\tightlist
\item
  A more careful calculation will need to take into account that \(\hat m_h(x)\)
...
\tightlist
\item
  The Nadaraya-Watson estimator provides a kernel-based method for
  nonparametric regression, estimating \(m(x)\) as a weighted average of
  nearby observations.
\item
  For kernels with bounded support, the estimator may be undefined
  when data points are too sparse (bandwidth too small).
\item
  Under a random design model, the estimator has bias of order \(O(h^2)\)
  and variance of order \(O(1/(nh))\).
\item
  The optimal bandwidth is \(h \sim n^{-1/5}\), giving mean squared error
  of order \(O(n^{-4/5})\).
\item
  The bias depends on both \(m''(x)\) and the ratio \(f'(x)/f(x)\), reflecting
  curvature in the regression function and the density of observations.
\item
  The variance is smaller in regions where observations are plentiful
  (large \(f(x)\)) and increases with the local noise level \(\sigma^2(x)\).
\end{itemize}
...
Nadaraya-Watson estimator that addresses some of its limitations,
particularly boundary bias. The method combines the two ideas of
...
estimates the regression coefficients by minimising the residual sum of
squares:
\begin{equation*}
  r(\beta)
...
\begin{equation*}
  ...
  = \tilde x^\top \hat\beta,
\end{equation*}
where \(\tilde x = (1, \tilde x_1, \ldots, \tilde x_p)\).

\subsection{Polynomial Regression}\label{polynomial-regression}

In these notes we only consider regression with a single input variable, \(x \in \mathbb{R}\).
In this case we can easily fit a polynomial of degree \(p\) to the data by using
...
The resulting regression line seems like a reasonable fit for the data.
...
\end{equation*}
where \(\tilde x\) is inside the interval of \(x\) values.
This leads to the design matrix
\begin{equation}
  X
  = \begin{pmatrix}
...
\end{equation*}
where the coefficients \(\hat\beta\) depend on \(\tilde x\). The model mean at
...
\begin{example}
\protect\hypertarget{exm:local-linear-implementation}{}\label{exm:local-linear-implementation}To better understand how the local linear estimator works, we can implement
it directly from the formulas derived above. Recall from earlier in this
section that \(\hat m_h(\tilde x) = \hat\beta_0\), where \(\hat\beta\) is
computed via weighted least squares. For the local linear case (\(p=1\)), the
matrix formula \(\hat\beta = (X^\top W X)^{-1} X^\top W y\) leads to the expression
\begin{equation*}
  \hat m_h(\tilde x)
  = \frac{T_1 T_2 - T_3 T_4}{B_1 B_2 - B_3^2},
\end{equation*}
where the seven terms are sums involving the kernel weights.

Here is a direct implementation in R:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Local linear regression estimator}
\NormalTok{local\_linear }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x\_train, y\_train, x\_eval, h) \{}
  \CommentTok{\# Matrix of differences: dx[i,j] = x\_eval[j] {-} x\_train[i]}
  \CommentTok{\# Each column corresponds to one evaluation point x\_eval[j]}
\NormalTok{  dx }\OtherTok{\textless{}{-}} \SpecialCharTok{{-}}\FunctionTok{outer}\NormalTok{(x\_train, x\_eval, }\StringTok{"{-}"}\NormalTok{)}

  \CommentTok{\# Kernel weights K\_h(x\_eval[j] {-} x\_train[i])}
\NormalTok{  K }\OtherTok{\textless{}{-}} \FunctionTok{dnorm}\NormalTok{(dx, }\AttributeTok{sd =}\NormalTok{ h)}

  \CommentTok{\# The seven terms from the weighted regression formula:}
\NormalTok{  T1 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(y\_train }\SpecialCharTok{*}\NormalTok{ K)}
\NormalTok{  T2 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(x\_train }\SpecialCharTok{*}\NormalTok{ (}\SpecialCharTok{{-}}\NormalTok{dx) }\SpecialCharTok{*}\NormalTok{ K)}
\NormalTok{  T3 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(x\_train }\SpecialCharTok{*}\NormalTok{ y\_train }\SpecialCharTok{*}\NormalTok{ K)}
\NormalTok{  T4 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{((}\SpecialCharTok{{-}}\NormalTok{dx) }\SpecialCharTok{*}\NormalTok{ K)}
\NormalTok{  B1 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(K)}
\NormalTok{  B2 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(x\_train}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{*}\NormalTok{ K)}
\NormalTok{  B3 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(x\_train }\SpecialCharTok{*}\NormalTok{ K)}

  \CommentTok{\# The local linear estimate at each evaluation point}
\NormalTok{  (T1}\SpecialCharTok{*}\NormalTok{T2 }\SpecialCharTok{{-}}\NormalTok{ T3}\SpecialCharTok{*}\NormalTok{T4) }\SpecialCharTok{/}\NormalTok{ (B1}\SpecialCharTok{*}\NormalTok{B2 }\SpecialCharTok{{-}}\NormalTok{ B3}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

We can test this implementation on a simple dataset:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{20251114}\NormalTok{)}
...
\CommentTok{\# Compute local linear estimate on a grid}
\NormalTok{x\_grid }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{length.out =} \DecValTok{100}\NormalTok{)}
\NormalTok{m\_ll }\OtherTok{\textless{}{-}} \FunctionTok{local\_linear}\NormalTok{(x, y, x\_grid, }\AttributeTok{h =} \FloatTok{0.05}\NormalTok{)}

\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{cex =}\NormalTok{ .}\DecValTok{5}\NormalTok{, }\AttributeTok{main =} \StringTok{"Local Linear Estimator (from scratch)"}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid, m\_ll, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{MATH5714M_files/figure-latex/locpoly0-1.pdf}}

The implementation shows clearly how the kernel weights \(K_h\) combine with
the data to produce the estimate at each point.
\end{example}
...
and of \texttt{locpoly(..,\ degree\ =\ 0,\ bandwidth\ =\ 0.11)} is nearly identical.
...
For \(p=2\) the local polynomials are parabolas. This sometimes allows us to reduce
bias near peaks.
...
\tightlist
\item
  Local polynomial regression combines weighted least squares with polynomial
  models, using kernel weights \(w_i = K_h(\tilde x - x_i)\) to emphasise nearby
  observations.
\item
  The estimate at \(\tilde x\) is \(\hat m_h(\tilde x) = \hat\beta_0\) where
  \(\hat\beta = (X^\top W X)^{-1} X^\top W y\) must be computed separately for
  each evaluation point.
\item
  For \(p=0\) (constant fit), the method reduces to the Nadaraya-Watson estimator;
  for \(p=1\) (local linear), it reduces boundary bias; for \(p=2\) (local quadratic),
  it reduces bias near peaks.
\item
  Local linear regression (\(p=1\)) performs better than Nadaraya-Watson at
  boundaries because the linear fit adapts to local slope.
\item
  In R, the \texttt{locpoly()} function from \texttt{KernSmooth} provides efficient
  implementations, though the method can be implemented directly from the
  weighted regression formula.
\end{itemize}
...
\textbf{5.} Consider the Nadaraya-Watson estimator
\begin{equation*}
  \hat m_h(x)
  = \frac{\sum_{i=1}^n K_h(x - x_i) y_i}{\sum_{j=1}^n K_h(x - x_j)},
\end{equation*}
where the Gaussian kernel is used for \(K\). Assuming the \(x_i\) are distinct,
show that \(\hat m_h(x_i) \to y_i\) as \(h \downarrow 0\) for each
data point~\(x_i\).

\begin{myanswers}
Evaluating the estimator at the data point \(x_i\), we have
\begin{equation*}
  \hat m_h(x_i)
  = \frac{\sum_{j=1}^n K_h(x_i - x_j) y_j}{\sum_{j=1}^n K_h(x_i - x_j)}
  = \frac{K_h(0) y_i + \sum_{j \neq i} K_h(x_i - x_j) y_j}{K_h(0) + \sum_{j \neq i} K_h(x_i - x_j)}.
\end{equation*}
Dividing both numerator and denominator by \(K_h(0) = \frac{1}{h} K(0)\) gives
\begin{equation*}
  \hat m_h(x_i)
  = \frac{y_i + \sum_{j \neq i} \frac{K_h(x_i - x_j)}{K_h(0)} y_j}{1 + \sum_{j \neq i} \frac{K_h(x_i - x_j)}{K_h(0)}}.
\end{equation*}
For \(j \neq i\) we have \(x_i - x_j \neq 0\), and thus \(|(x_i - x_j)/h| \to \infty\) as
\(h\downarrow 0\). For the Gaussian kernel, \(K(u) = \frac{1}{\sqrt{2\pi}}
e^{-u^2/2} \to 0\) as \(|u| \to \infty\). Therefore
\begin{equation*}
  \frac{K_h(x_i - x_j)}{K_h(0)}
  = \frac{K\bigl((x_i - x_j)/h\bigr)}{K(0)}
  \to \frac{0}{K(0)}
  = 0
\end{equation*}
as \(h \downarrow 0\), using \(K(0) > 0\). Since there are only finitely many terms
in the sums (namely \(n-1\) terms), we conclude that
\begin{equation*}
  \hat m_h(x_i)
  \to \frac{y_i + 0}{1 + 0}
  = y_i
...
\textbf{6.} The bias of the Nadaraya-Watson estimator satisfies
\begin{equation*}
  \mathop{\mathrm{bias}}\bigl(\hat m_h(x)\bigr)
  \approx \frac{\mu_2(K) f'(x)}{f(x)} m'(x) h^2 + \frac{\mu_2(K)}{2} m''(x) h^2,
\end{equation*}
while for the local linear estimator (section~\ref{X05-locpoly}), the bias is
\begin{equation*}
  \mathop{\mathrm{bias}}\approx \frac{\mu_2(K)}{2} m''(x) h^2
\end{equation*}
for small~\(h\).
...
\tightlist
\item
  Suppose observations are more dense to the left of a point \(x_0\) than they
  are to the right, \emph{i.e.}~\(f'(x_0) < 0\), and that the regression function~\(m\) has
  positive slope at \(x_0\). Explain geometrically why the Nadaraya-Watson
  estimator underestimates \(m(x_0)\).
\end{enumerate}

\begin{myanswers}
The Nadaraya-Watson estimator computes a weighted average
\(\hat m_h(x_0) = \sum_i w_i(x_0) y_i\), where the weights are determined by
the distance from the point \(x_0\).
Since \(m\) has positive slope, the \(y\)-values to the left of \(x_0\) are smaller than \(m(x_0)\),
while those to the right are larger.
When observations are more dense to the left, more data points lie there and
receive collectively greater weight. The weighted average is therefore pulled
downward, causing underestimation.
...
\tightlist
\item
  Let \(m(x) = 2x\) and \(f(x) = 2(1-x)\) for \(x \in [0,1]\). Compute the
  Nadaraya-Watson bias at \(x_0 = 0.5\) as a multiple of \(\mu_2(K) h^2\).
\end{enumerate}

\begin{myanswers}
We have \(m'(x_0) = 2\), \(m''(x_0) = 0\), \(f(0.5) = 1\), and \(f'(0.5) = -2\). The
Nadaraya-Watson bias is
\begin{equation*}
  \mathop{\mathrm{bias}}\bigl(\hat m_h(0.5)\bigr)
  \approx \frac{\mu_2(K) \cdot (-2) \cdot 2}{1} h^2 + \frac{\mu_2(K)}{2} \cdot 0 \cdot h^2
  = -4 \mu_2(K) h^2.
\end{equation*}
The negative sign confirms the underestimation predicted in part (a).
...
\tightlist
\item
  Show that the bias for local linear regression at the same point equals zero.
  Explain geometrically why
  fitting a local line, rather than taking a local average, eliminates this
  bias.
\end{enumerate}

\begin{myanswers}
The local linear bias is
\begin{equation*}
  \mathop{\mathrm{bias}}\approx \frac{\mu_2(K)}{2} \cdot 0 \cdot h^2 = 0.
\end{equation*}
Local linear regression fits a line to the weighted observations rather than
averaging them. Even when observations are concentrated on one side of \(x_0\),
the fitted line can extrapolate to \(x_0\) using its estimated slope. Since the
true function \(m(x) = 2x\) is linear, the locally fitted line matches \(m\)
exactly in expectation, eliminating the bias.

\end{myanswers}

\textbf{7.} Consider local linear regression with kernel weights
\(W = \mathrm{diag}(K_h(\tilde x - x_1), \ldots, K_h(\tilde x - x_n))\) and design
matrix \(X\) with rows \((1, x_i - \tilde x)\). The local linear estimate
for \(m(\tilde x)\) can be written as
\begin{equation*}
  \hat m_h(\tilde x)
  = e_0^\top (X^\top W X)^{-1} X^\top W y,
\end{equation*}
...
\tightlist
\item
  Show that \(\hat m_h(\tilde x) = \sum_{i=1}^n w_i y_i\) for weights \(w_i\) that
  do not depend on the \(y_j\).
\end{enumerate}

\begin{myanswers}
The expression \(e_0^\top (X^\top W X)^{-1} X^\top W y\) is linear in \(y\). Writing
\(w^\top = e_0^\top (X^\top W X)^{-1} X^\top W\), we have
\(\hat m_h(\tilde x) = w^\top y = \sum_{i=1}^n w_i y_i\). The vector \(w\) depends
on \(\tilde x\) and the \(x_i\) (through \(X\) and \(W\)) but not on the \(y_j\).
...
\tightlist
\item
  Show that if \(y_i = a + b(x_i - \tilde x)\) for constants \(a\) and \(b\), then
  \(\hat m_h(\tilde x) = a\).
\end{enumerate}

\begin{myanswers}
If \(y_i = a + b(x_i - \tilde x)\), then \(\beta_0 = a\), \(\beta_1 = b\) makes every
residual \(y_i - \beta_0 - \beta_1(x_i - \tilde x)\) equal to zero. The weighted
RSS therefore vanishes, which is its minimum, so \(\hat\beta = (a, b)^\top\) and
\(\hat m_h(\tilde x) = \hat\beta_0 = a\).
...
\tightlist
\item
  Deduce that \(\sum_{i=1}^n w_i = 1\) and
  \(\sum_{i=1}^n w_i(x_i - \tilde x) = 0\).
\end{enumerate}

\begin{myanswers}
From part (a), \(\hat m_h(\tilde x) = \sum w_i y_i\) where the weights \(w_i\) do not
depend on the \(y_j\). Thus we can choose \(y_i = a + b(x_i - \tilde x)\) as in
part (b), giving \(\hat m_h(\tilde x) = a\). This gives
\begin{equation*}
  a
  = \hat m_h(\tilde x)
  = \sum_{i=1}^n w_i (a + b(x_i - \tilde x))
  = a \sum_{i=1}^n w_i + b \sum_{i=1}^n w_i(x_i - \tilde x).
\end{equation*}
Rearranging gives
\begin{equation*}
  a\Bigl( \sum_{i=1}^n w_i - 1 \Bigr) + b \sum_{i=1}^n w_i(x_i - \tilde x) = 0.
\end{equation*}
Since this holds for all \(a, b\), both coefficients must vanish: \(\sum w_i = 1\)
and \(\sum w_i(x_i - \tilde x) = 0\).

\end{myanswers}

\clearpage

\section{\texorpdfstring{\emph{k}-Nearest Neighbour Regression}{k-Nearest Neighbour Regression}}\label{X06-nearest}
...
Later, in section~\ref{X08-xval} on cross-validation, we will learn
how \(k\) can be chosen from data.
...
\tightlist
\item
  The \(k\)-NN estimator averages the \(k\) nearest responses, providing an
  adaptive bandwidth that adjusts to local data density.
\item
  The parameter \(k\) controls smoothness: larger \(k\) gives smoother estimates,
  while \(k=1\) produces a step function matching the nearest observation.
\item
  The method extends naturally to higher dimensions \(\mathbb{R}^p\), though finding
  nearest neighbours becomes computationally expensive without specialised
  data structures.
\item
  In R, the \texttt{FNN} package provides efficient implementations via \texttt{knn.reg()}.
\item
  Variants include symmetric \(k\)-NN and continuous versions using local
  bandwidth \(h(x)\) based on the distance to the \(k\)th nearest neighbour.
\end{itemize}

\clearpage

\section{Spline Smoothing}\label{X07-splines}

The previous sections showed how kernel methods and \(k\)-nearest neighbour
methods estimate the regression function \(m\colon \mathbb{R}\to \mathbb{R}\) from data \((x_1,
y_1), \ldots, (x_n, y_n)\). In this section we introduce an alternative approach
based on \textbf{spline smoothing}. Instead of local averaging, we fit a smooth
function globally to the entire dataset, using a penalty term to control the
smoothness.
...
\tightlist
\item
  Fit the data well: minimise the residual sum of
  squares \(\sum_{i=1}^n \bigl( y_i - m(x_i) \bigr)^2\).
\item
  Remain smooth: avoid excessive curvature.
\end{enumerate}

We measure the curvature of a function \(m\) using the integral of the
squared second derivative:
\begin{equation*}
  \int_{-\infty}^\infty \bigl( m''(x) \bigr)^2 \,dx.
\end{equation*}
A function with high curvature produces a large integral; a nearly linear
function produces a small integral. A linear function \(m(x) = a + bx\) has
\(m''(x) = 0\), giving an integral of zero.
...
function \(\hat m_\lambda\) which minimises
\begin{equation}
  I(m)
...
\begin{example}
Consider the knots at \(\kappa_1 = 0\), \(\kappa_2 = 1\), and \(\kappa_3 = 2\). We
will show that the function defined by
...
is a natural cubic spline, by checking values and continuity at the
knots. For \(\kappa_1 = 0\) we have:
...
The following theorem shows that the solution to the smoothing spline problem
is always a natural cubic spline.
...
The
\href{https://en.wikipedia.org/wiki/Calculus_of_variations}{``calculus of variations''}
shows that among all twice-differentiable functions \(f\colon [x_i,
x_{i+1}] \to \mathbb{R}\) with given values and derivatives at the endpoints, the cubic
polynomial minimises \(\int_{x_i}^{x_{i+1}} [f''(x)]^2 dx\). Equality holds only
if \(\hat m_\lambda\) is already cubic on \([x_i, x_{i+1}]\).

Now suppose \(\hat m_\lambda\) is the solution but \(\hat m_\lambda\) is not a
cubic polynomial on some interval \([x_i, x_{i+1}]\). We will show that this
leads to a contradiction.

Let \(p\) be the unique cubic polynomial on \([x_i, x_{i+1}]\) which matches \(\hat
m_\lambda\) in value and first derivative at both endpoints: \(p(x_i) = \hat
m_\lambda(x_i)\), \(p'(x_i) = \hat m_\lambda'(x_i)\), \(p(x_{i+1}) = \hat
m_\lambda(x_{i+1})\), and \(p'(x_{i+1}) = \hat m_\lambda'(x_{i+1})\). Now consider
the function \(\tilde m\) which equals \(\hat m_\lambda\) outside \([x_i, x_{i+1}]\)
and equals \(p\) on \([x_i, x_{i+1}]\).

Since \(\tilde m\) agrees with \(\hat m_\lambda\) at all data points,
both functions produce the same residual sum of squares. However, \(\tilde m\)
has a smaller penalty term:
\begin{equation*}
  \int_{-\infty}^\infty \bigl( \tilde m''(x) \bigr)^2 \,dx
  < \int_{-\infty}^\infty \bigl( \hat m_\lambda''(x) \bigr)^2 \,dx,
\end{equation*}
by the property of cubic polynomials mentioned above. This contradicts the
assumption that \(\hat m_\lambda\) minimises equation~\eqref{eq:spline-penalty}.
Therefore, \(\hat m_\lambda\) must be a cubic polynomial on each interval \([x_i,
x_{i+1}]\).
...
The theorem shows that the smoothing spline is a natural cubic spline with \(n\)
knots at the data points \(x_1, \ldots, x_n\). Such a spline consists of:
...
This gives \(2 + 4(n-1) + 2 = 4n\) parameters initially. However, at each of the
\(n\) knots, we require continuity of the function, its first derivative, and its
second derivative. This imposes \(3n\) constraints, leaving \(4n - 3n = n\) free
parameters.
With \(n\) free parameters and \(n\) data points \((x_1, y_1), \ldots,
(x_n, y_n)\), we could make the spline pass through all
data points exactly---a perfect interpolating fit.
This corresponds to \(\lambda = 0\). For \(\lambda > 0\), the penalty
term in equation~\eqref{eq:spline-penalty} forces the spline to be smoother,
in exchange for imperfect fit.
...
responses \(y_1, \ldots, y_n\). We omit the explicit formula for
\(S_\lambda\), but note that efficient algorithms exist for computing it.
...
the amount of smoothing: choose a target value for \(\mathop{\mathrm{df}}\nolimits(\lambda)\) and
...
\href{https://rdrr.io/r/stats/smooth.spline.html}{\texttt{smooth.spline()}}, which
...
\item
  ...
  a different scale: specifically, \(\lambda = r \cdot 256^{3 \cdot \text{spar} - 1}\)
  where \(r\) is a data-dependent scaling factor. If not specified, the function
...
\item
  ...
  grid of points if you set the optional argument \texttt{all.knots\ =\ FALSE}).
\item
  \texttt{\$df}: the effective degrees of freedom of the fitted spline.
\item
  \texttt{\$lambda}: the smoothing parameter \(\lambda\).
\end{itemize}

\begin{example}
We illustrate smoothing splines using the \texttt{faithful} dataset, which contains
...
\NormalTok{x }\OtherTok{\textless{}{-}}\NormalTok{ faithful}\SpecialCharTok{$}\NormalTok{waiting}
\NormalTok{y }\OtherTok{\textless{}{-}}\NormalTok{ faithful}\SpecialCharTok{$}\NormalTok{eruptions}

\CommentTok{\# Sort the data for plotting}
...
\CommentTok{\# Fit smoothing splines with different df}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{))}

\CommentTok{\# Left panel: varying degrees of freedom}
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{main =} \StringTok{"Smoothing Splines with Different df"}\NormalTok{,}
     \AttributeTok{xlab =} \StringTok{"Waiting time (min)"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Duration (min)"}\NormalTok{,}
     \AttributeTok{pch =} \DecValTok{16}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.7}\NormalTok{)}

\CommentTok{\# Fit splines with df = 3, 6, and 15}
\NormalTok{fit3 }\OtherTok{\textless{}{-}} \FunctionTok{smooth.spline}\NormalTok{(x, y, }\AttributeTok{df =} \DecValTok{3}\NormalTok{)}
\NormalTok{fit6 }\OtherTok{\textless{}{-}} \FunctionTok{smooth.spline}\NormalTok{(x, y, }\AttributeTok{df =} \DecValTok{6}\NormalTok{)}
\NormalTok{fit15 }\OtherTok{\textless{}{-}} \FunctionTok{smooth.spline}\NormalTok{(x, y, }\AttributeTok{df =} \DecValTok{15}\NormalTok{)}

\FunctionTok{lines}\NormalTok{(fit3, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(fit6, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(fit15, }\AttributeTok{col =} \StringTok{"darkgreen"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}

\FunctionTok{legend}\NormalTok{(}\StringTok{"bottomright"}\NormalTok{,}
       \AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"df = 3"}\NormalTok{, }\StringTok{"df = 6"}\NormalTok{, }\StringTok{"df = 15"}\NormalTok{),}
...
\CommentTok{\# Right panel: comparison with kernel smoother}
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{main =} \StringTok{"Spline vs. Kernel Smoother"}\NormalTok{,}
     \AttributeTok{xlab =} \StringTok{"Waiting time (min)"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Duration (min)"}\NormalTok{,}
     \AttributeTok{pch =} \DecValTok{16}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.7}\NormalTok{)}

\CommentTok{\# Smoothing spline with df = 15}
\FunctionTok{lines}\NormalTok{(fit15, }\AttributeTok{col =} \StringTok{"darkgreen"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}

\CommentTok{\# Nadaraya{-}Watson kernel smoother with automatic bandwidth}
\NormalTok{h }\OtherTok{\textless{}{-}} \FunctionTok{bw.nrd}\NormalTok{(x)}
\NormalTok{fit\_nw }\OtherTok{\textless{}{-}} \FunctionTok{ksmooth}\NormalTok{(x, y, }\AttributeTok{kernel =} \StringTok{"normal"}\NormalTok{, }\AttributeTok{bandwidth =}\NormalTok{ h)}
\FunctionTok{lines}\NormalTok{(fit\_nw, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}

\FunctionTok{legend}\NormalTok{(}\StringTok{"bottomright"}\NormalTok{,}
       \AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Spline (df = 15)"}\NormalTok{,}
                  \FunctionTok{paste}\NormalTok{(}\StringTok{"Nadaraya{-}Watson (h ="}\NormalTok{, }\FunctionTok{round}\NormalTok{(h, }\DecValTok{1}\NormalTok{), }\StringTok{")"}\NormalTok{)),}
       \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"darkgreen"}\NormalTok{, }\StringTok{"blue"}\NormalTok{),}
       \AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{MATH5714M_files/figure-latex/faithful-splines-1.pdf}}

The left panel shows how degrees of freedom control smoothness: \(\mathop{\mathrm{df}}\nolimits= 3\)
gives an almost linear fit, while \(\mathop{\mathrm{df}}\nolimits= 15\) follows the data closely.
The right panel compares the smoothing spline (\(\mathop{\mathrm{df}}\nolimits= 15\)) with the
Nadaraya-Watson estimator (using the normal reference rule for bandwidth
selection). Both methods produce similar estimates, though the spline
has better boundary behaviour.
\end{example}

\subsection{Comparison with Kernel Methods}\label{comparison-with-kernel-methods}

Both smoothing splines and kernel methods estimate the
regression function \(m\) from data, but they differ in
...
\tightlist
\item
  \textbf{Global method:} The spline fits the entire dataset at once,
  which gives better results at the boundaries of the data range.
\item
  \textbf{Automatic smoothness:} The solution is always twice
  continuously differentiable.
\item
  \textbf{Efficient computation:} Solving a system of linear equations
  computes the spline, avoiding local fits at many points.
\end{itemize}
...
\item
  ...
  (as in \(k\)-NN methods).
\item
  \textbf{Geometric interpretation:} Kernel methods use weighted averages,
  which are easier to understand than the penalized least squares
  formulation of splines.
\item
  \textbf{Robustness:} Robust kernels or trimmed extreme values make kernel
...
\begin{example}
We compare smoothing splines with Nadaraya-Watson and local linear regression
using simulated data where the true regression function is known. This allows
us to assess the performance of each method, particularly near the boundaries.
...
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{sort}\NormalTok{(}\FunctionTok{runif}\NormalTok{(n, }\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{))}
\NormalTok{m\_true }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(t) }\FunctionTok{sin}\NormalTok{(}\DecValTok{2}\SpecialCharTok{*}\NormalTok{t) }\SpecialCharTok{+} \DecValTok{2}\SpecialCharTok{*}\FunctionTok{exp}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{16}\SpecialCharTok{*}\NormalTok{(t}\DecValTok{{-}2}\NormalTok{)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\NormalTok{y }\OtherTok{\textless{}{-}} \FunctionTok{m\_true}\NormalTok{(x) }\SpecialCharTok{+} \FunctionTok{rnorm}\NormalTok{(n, }\DecValTok{0}\NormalTok{, }\FloatTok{0.3}\NormalTok{)}

\CommentTok{\# Grid for evaluation}
\NormalTok{x\_grid }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{, }\AttributeTok{length =} \DecValTok{200}\NormalTok{)}
\NormalTok{y\_true }\OtherTok{\textless{}{-}} \FunctionTok{m\_true}\NormalTok{(x\_grid)}

\CommentTok{\# Nadaraya{-}Watson estimator}
\NormalTok{nw\_estimate }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x\_new, x, y, h) \{}
\NormalTok{  K }\OtherTok{\textless{}{-}} \FunctionTok{dnorm}\NormalTok{((x\_new }\SpecialCharTok{{-}}\NormalTok{ x) }\SpecialCharTok{/}\NormalTok{ h)}
  \FunctionTok{sum}\NormalTok{(K }\SpecialCharTok{*}\NormalTok{ y) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(K)}
\NormalTok{\}}

\CommentTok{\# Local linear estimator}
\NormalTok{local\_linear }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x\_new, x, y, h) \{}
\NormalTok{  w }\OtherTok{\textless{}{-}} \FunctionTok{dnorm}\NormalTok{((x }\SpecialCharTok{{-}}\NormalTok{ x\_new) }\SpecialCharTok{/}\NormalTok{ h)}
\NormalTok{  X }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(}\DecValTok{1}\NormalTok{, x }\SpecialCharTok{{-}}\NormalTok{ x\_new)}
\NormalTok{  beta }\OtherTok{\textless{}{-}} \FunctionTok{solve}\NormalTok{(}\FunctionTok{t}\NormalTok{(X) }\SpecialCharTok{\%*\%} \FunctionTok{diag}\NormalTok{(w) }\SpecialCharTok{\%*\%}\NormalTok{ X) }\SpecialCharTok{\%*\%} \FunctionTok{t}\NormalTok{(X) }\SpecialCharTok{\%*\%} \FunctionTok{diag}\NormalTok{(w) }\SpecialCharTok{\%*\%}\NormalTok{ y}
\NormalTok{  beta[}\DecValTok{1}\NormalTok{]}
\NormalTok{\}}

\CommentTok{\# Choose bandwidth using rule of thumb}
\NormalTok{h }\OtherTok{\textless{}{-}} \FloatTok{0.3}

\CommentTok{\# Compute estimates}
\NormalTok{y\_nw }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(x\_grid, nw\_estimate, }\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y, }\AttributeTok{h =}\NormalTok{ h)}
\NormalTok{y\_ll }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(x\_grid, local\_linear, }\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y, }\AttributeTok{h =}\NormalTok{ h)}

\CommentTok{\# Smoothing spline with fixed degrees of freedom}
\NormalTok{fit\_spline }\OtherTok{\textless{}{-}} \FunctionTok{smooth.spline}\NormalTok{(x, y, }\AttributeTok{df =} \DecValTok{5}\NormalTok{)}
\NormalTok{y\_spline }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(fit\_spline, x\_grid)}\SpecialCharTok{$}\NormalTok{y}

\CommentTok{\# Plotting}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{))}

\CommentTok{\# Left panel: all three methods}
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{main =} \StringTok{"Comparison of Smoothing Methods"}\NormalTok{,}
     \AttributeTok{xlab =} \StringTok{"x"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"y"}\NormalTok{, }\AttributeTok{pch =} \DecValTok{16}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.7}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid, y\_true, }\AttributeTok{col =} \StringTok{"black"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid, y\_nw, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid, y\_ll, }\AttributeTok{col =} \StringTok{"darkgreen"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid, y\_spline, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}

\FunctionTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{,}
       \AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"True function"}\NormalTok{, }\StringTok{"Nadaraya{-}Watson"}\NormalTok{,}
                  \StringTok{"Local linear"}\NormalTok{, }\FunctionTok{paste}\NormalTok{(}\StringTok{"Spline (df ="}\NormalTok{, }\FunctionTok{round}\NormalTok{(fit\_spline}\SpecialCharTok{$}\NormalTok{df, }\DecValTok{1}\NormalTok{), }\StringTok{")"}\NormalTok{)),}
       \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"black"}\NormalTok{, }\StringTok{"blue"}\NormalTok{, }\StringTok{"darkgreen"}\NormalTok{, }\StringTok{"red"}\NormalTok{),}
       \AttributeTok{lty =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{bty =} \StringTok{"n"}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.8}\NormalTok{)}

\CommentTok{\# Right panel: zoom on boundary region}
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{main =} \StringTok{"Boundary Behaviour (x \textless{} 0.5)"}\NormalTok{,}
     \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{), }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{2}\NormalTok{),}
     \AttributeTok{xlab =} \StringTok{"x"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"y"}\NormalTok{, }\AttributeTok{pch =} \DecValTok{16}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey70"}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.7}\NormalTok{)}

\CommentTok{\# Only plot in boundary region}
\NormalTok{idx }\OtherTok{\textless{}{-}}\NormalTok{ x\_grid }\SpecialCharTok{\textless{}=} \FloatTok{0.5}
\FunctionTok{lines}\NormalTok{(x\_grid[idx], y\_true[idx], }\AttributeTok{col =} \StringTok{"black"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid[idx], y\_nw[idx], }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid[idx], y\_ll[idx], }\AttributeTok{col =} \StringTok{"darkgreen"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(x\_grid[idx], y\_spline[idx], }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{)}

\CommentTok{\# Add vertical line at x = h to show bandwidth}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v =}\NormalTok{ h, }\AttributeTok{col =} \StringTok{"grey"}\NormalTok{, }\AttributeTok{lty =} \DecValTok{3}\NormalTok{)}
\FunctionTok{text}\NormalTok{(h, }\FloatTok{0.2}\NormalTok{, }\StringTok{"h"}\NormalTok{, }\AttributeTok{pos =} \DecValTok{4}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{MATH5714M_files/figure-latex/comparison-methods-1.pdf}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Boundary effects}: The Nadaraya-Watson estimator (blue) shows clear bias
  near \(x = 0\), underestimating the function. The local linear estimator
  (green) performs better at the boundary but still shows bias. The
  smoothing spline (red) handles the boundary well through its global nature.
\item
  \textbf{Peak capture}: All methods struggle with the sharp peak at
  \(x = 2\), but the smoothing spline adapts well despite using a
  global smoothing parameter.
\item
  \textbf{Smoothness}: The spline produces the smoothest estimate, constrained
  to be twice differentiable everywhere. The kernel methods
  show more local variation.
\end{enumerate}

The choice of method depends on the specific
requirements---boundary behaviour, local adaptation,
or global smoothness.
\end{example}

Methods for selecting the smoothing parameter \(\lambda\) or the effective
degrees of freedom using cross-validation are discussed in
section~\ref{X08-xval}.
...
\tightlist
\item
  Smoothing splines balance fit and smoothness using a penalty term on curvature.
\item
  The solution is a natural cubic spline with knots at the data points.
\item
  Smoothness is controlled by \(\lambda\) or equivalently by effective degrees of freedom.
\item
  Compared to kernel methods, splines are global and handle boundaries better.
\end{itemize}
...
\textbf{8.} The \(k\)-nearest neighbour estimator is defined as
\begin{equation*}
  \hat m_k(x)
  = \frac{1}{k} \sum_{i \in J_k(x)} y_i,
\end{equation*}
where \(J_k(x) = \bigl\{ i \bigm| \text{$x_i$ is one of the $k$ nearest
observations to $x$} \bigr\}\). Assuming the \(x_i\) are distinct, show that
\(\hat m_1(x_j) = y_j\) for each data point \(x_j\).

\begin{myanswers}
For \(k = 1\), the set \(J_1(x_j)\) contains the single index \(i\) for which \(x_i\) is
closest to \(x_j\). Since the \(x_i\) are distinct, the closest point to \(x_j\) is
\(x_j\) itself (with distance zero). Thus \(J_1(x_j) = \{j\}\) and
\begin{equation*}
  \hat m_1(x_j)
  = \frac{1}{1} \sum_{i \in \{j\}} y_i
  = y_j.
\end{equation*}

\end{myanswers}

\textbf{9.} Consider the \(k\)-nearest neighbour estimator with \(k = 1\) and ordered data
\(x_1 < x_2 < \cdots < x_n\).
...
\tightlist
\item
  For \(x \in (x_i, x_{i+1})\), find the value of \(x\) at which the nearest
  neighbour changes from \(x_i\) to \(x_{i+1}\).
\end{enumerate}

\begin{myanswers}
The nearest neighbour is \(x_i\) when \(|x - x_i| < |x - x_{i+1}|\), \emph{i.e.}~when
\(x - x_i < x_{i+1} - x\), which simplifies to \(x < \frac{x_i + x_{i+1}}{2}\).
Similarly, the nearest neighbour is \(x_{i+1}\) when \(x > \frac{x_i + x_{i+1}}{2}\).
Thus the nearest neighbour changes at the midpoint \(\frac{x_i + x_{i+1}}{2}\).
...
\tightlist
\item
  Deduce that \(\hat m_1\) is a step function, and state the intervals on which
  it is constant.
\end{enumerate}

\begin{myanswers}
From part (a), the nearest neighbour changes only at the midpoints between
consecutive data points. For \(x < x_1\), the nearest neighbour is \(x_1\); for
\(x > x_n\), it is \(x_n\). Thus \(\hat m_1\) is constant on each of the \(n\) intervals
\begin{equation*}
  \Bigl( -\infty, \frac{x_1 + x_2}{2} \Bigr), \quad
  \Bigl( \frac{x_1 + x_2}{2}, \frac{x_2 + x_3}{2} \Bigr), \quad \ldots, \quad
  \Bigl( \frac{x_{n-1} + x_n}{2}, \infty \Bigr),
\end{equation*}
taking values \(y_1, y_2, \ldots, y_n\) respectively. This makes \(\hat m_1\) a step
function with jumps at the \(n - 1\) midpoints.

\end{myanswers}

\textbf{10.} Among smooth interpolants, the natural cubic spline minimises roughness
\(\int (m''(x))^2 \, dx\). This question illustrates this for two points within a
family of cubics.

Consider fitting the points \((0, 0)\) and \((1, 1)\). For any constants \(p\) and \(q\),
the cubic
\begin{equation*}
  m(x) = x + x(1-x)\bigl(p(1-x) + qx\bigr)
\end{equation*}
passes through both points, with the linear function \(m(x) = x\) corresponding
to \(p = q = 0\).
...
\tightlist
\item
  Show that the roughness \(\int_0^1 (m''(x))^2 \, dx\) equals \(4(p^2 - pq + q^2)\).
\end{enumerate}

\begin{myanswers}
Expanding \(m(x)\):
\begin{equation*}
  m(x) = (1+p)x + (q-2p)x^2 + (p-q)x^3.
\end{equation*}
Differentiating twice:
\begin{align*}
  m'(x) &= (1+p) + 2(q-2p)x + 3(p-q)x^2, \\
  m''(x) &= 2(q-2p) + 6(p-q)x.
\end{align*}
Writing \(m''(x) = \alpha + \beta x\) with \(\alpha = 2(q-2p)\) and \(\beta = 6(p-q)\),
\begin{equation*}
  \int_0^1 (m''(x))^2 \, dx
  = \int_0^1 (\alpha + \beta x)^2 \, dx
  = \alpha^2 + \alpha\beta + \frac{\beta^2}{3}.
\end{equation*}
Substituting \(\alpha = 2q - 4p\) and \(\beta = 6p - 6q\) and simplifying gives
\(4(p^2 - pq + q^2)\).
...
\tightlist
\item
  By completing the square (or otherwise), show that \(p^2 - pq + q^2 \geq 0\)
  with equality if and only if \(p = q = 0\).
\end{enumerate}
...
\begin{equation*}
  p^2 - pq + q^2
  = \Bigl(p - \frac{q}{2}\Bigr)^{\!2} + \frac{3q^2}{4}.
\end{equation*}
This is a sum of two squares, hence non-negative. Equality holds if and only if
both \(p - q/2 = 0\) and \(q = 0\), which requires \(p = q = 0\).
...
\tightlist
\item
  Since every member of this family passes through both data points,
  \(\mathrm{RSS} = 0\) for each. Deduce that for any \(\lambda > 0\), the linear
  function \(m(x) = x\) uniquely minimises
  \(I(m) = \mathrm{RSS} + \lambda \int_0^1 (m''(x))^2 \, dx\) within this family.
\end{enumerate}

\begin{myanswers}
With \(\mathrm{RSS} = 0\), the objective becomes
\begin{equation*}
  I(m) = \lambda \int_0^1 (m''(x))^2 \, dx = 4\lambda(p^2 - pq + q^2).
\end{equation*}
For \(\lambda > 0\), minimising \(I(m)\) is equivalent to minimising \(p^2 - pq + q^2\).
By part (b), this is minimised uniquely at \(p = q = 0\), giving \(m(x) = x\).
...
In this section we will discuss methods for choosing the bandwidth~\(h\)
for kernel-based methods, the size \(k\) of the neighbourhood used
in \(k\)-nearest neighbour regression, and the smoothing parameter \(\lambda\)
(or equivalently the degrees of freedom) for spline smoothing. The methods
we will use here are based on cross-validation.
...
This way, a method cannot achieve a misleadingly good fit by overfitting the data.
...
\end{equation*}
instead,
where \(\hat m^{(i)}\) is the kernel regression estimate computed without using
sample~\(i\):
\begin{equation*}
  \hat m_h^{(i)}(x)
...
For spline smoothing (see section~\ref{X07-splines}), we need to choose
the smoothing parameter \(\lambda\) or, equivalently, the effective degrees of
freedom. The same leave-one-out cross-validation approach can be used:
we minimise
\begin{equation*}
  r_\mathrm{LOO}(\lambda)
  := \sum_{i=1}^n \bigl( y_i - \hat s^{(i)}_\lambda(x_i) \bigr)^2,
\end{equation*}
where \(\hat s^{(i)}_\lambda\) is the smoothing spline fitted to the data
with sample~\(i\) omitted.

In R, the \texttt{smooth.spline()} function provides automatic parameter selection
via cross-validation when the argument \texttt{cv\ =\ TRUE} is used:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fit smoothing spline with CV{-}selected smoothing parameter}
\NormalTok{fit }\OtherTok{\textless{}{-}} \FunctionTok{smooth.spline}\NormalTok{(x, y, }\AttributeTok{cv =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

By default, \texttt{smooth.spline()} uses ordinary (leave-one-out) cross-validation
to select the effective degrees of freedom.
...
leave-one-out likelihood, which is given by
\begin{equation*}
  L_\mathrm{LOO}(h;x_1, \ldots, x_n)
  = \prod_{i=1}^n \hat f^{(i)}_h(x_i).
\end{equation*}
This technique is known as \textbf{maximum likelihood cross-validation}. When this
method is used in practice, it is advantageous to maximise the logarithm
\begin{align*}
...
\item
  Let \(\hat f_h^{(i)}\) be the kernel density estimate computed from the data
  with sample \(i\) omitted.
  Then
  \begin{equation*}
    \mathrm{CV}(h)
...
\begin{proof}
The first statement can be shown by expanding the square in the
...
Since the estimator \(\hat f_h^{(1)}\) is computed from \(X_2, \ldots, X_n\),
which are independent of \(X_1\), we can evaluate the expectation on
...
the expectation over \(X_1\) afterwards. This gives
\begin{align*}
  \mathbb{E}\Bigl( \frac{1}{n} \sum_{i=1}^n \hat f_h^{(i)}(X_i) \Bigr)
...
\tightlist
\item
  Minimising residual sum of squares directly leads to \(h = 0\) (overfitting);
  cross-validation avoids this by evaluating fitted values on held-out data.
\item
  In leave-one-out cross-validation, each observation is predicted using
  a model fitted without that observation; \(k\)-fold CV is a faster approximation.
\item
  For regression, we minimise \(r_\mathrm{LOO}(h) = \sum_i (y_i - \hat m_h^{(i)}(x_i))^2\).
\item
  For KDE, two approaches: maximum likelihood CV (maximise \(\sum_i \log \hat f_h^{(i)}(x_i)\))
  or integrated squared error CV (minimise \(\mathrm{CV}(h)\), which estimates IMSE).
\end{itemize}
...
In order to speed up the computation of the \(\hat f_h^{(i)}\), we implement
...
\item
  Term \(A\) in equation~\eqref{eq:CVh} can now be approximated by
...
\item
  ...
  then the result is the matrix \((k_{ij}y_i)_{i,j}\), \emph{i.e.} each row
...
Here we use the ``linear'' case, \(p=1\).
...
\(K_h(x-x_j)\) inside the sum, we can achieve this by setting the corresponding
...
optimal \(k\) for \(k\)-nearest neighbour regression. Unlike KDE and kernel
regression, where clever use of matrix operations allowed us to compute all
leave-one-out predictions efficiently, kNN offers no such shortcut: removing
observation \(i\) can change which points are the \(k\) nearest neighbours for
every other observation. We therefore resort to a nested loop over both \(k\)
values and observations, fitting a separate model for each combination. For
this reason, the code in this section is much slower to run than the code in
...
We note that the leave-one-out mean squared error for kNN is smaller
...
densities of \(x\)-values, it makes sense that a method which chooses
...
\tightlist
\item
  Cross-validation for kernel density estimation uses integrated squared error
  or maximum likelihood criteria to select bandwidth~\(h\).
\item
  For kernel regression, leave-one-out cross-validation minimises
  \(\sum_i (y_i - \hat m_h^{(i)}(x_i))^2\) where \(\hat m_h^{(i)}\) excludes
  observation~\(i\).
\item
  Efficient implementation uses matrix operations with \texttt{outer()} to compute
  all pairwise differences and kernel weights simultaneously.
\item
  The \texttt{faithful} dataset example shows that \(k\)-NN regression can outperform
  fixed-bandwidth methods when data density varies across the domain.
\item
  Optimal parameters differ between methods: local linear regression typically
  requires larger bandwidth than Nadaraya-Watson due to reduced boundary bias.
\end{itemize}

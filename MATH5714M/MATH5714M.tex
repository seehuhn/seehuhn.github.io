...
##  KDE1 1674.932 2401.0830 2645.4077 2434.190 2562.9920 17928.808  1000
##  KDE2  221.687  251.3505  392.7281  323.654  332.0385  8761.823  1000
##  KDE3  195.119  219.7805  375.1720  290.608  299.1770 14311.214  1000
\end{verbatim}
...
## Unit: microseconds
##     expr     min      lq     mean  median       uq      max neval
##  density 229.887 263.794 510.2636 291.387 368.9385 26324.87  1000
\end{verbatim}
...
\NormalTok{local\_linear }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x\_train, y\_train, x\_eval, h) \{}
  \CommentTok{\# Matrix of differences between evaluation points and training points}
  \CommentTok{\# Each column corresponds to one evaluation point x\_eval[j]}
\NormalTok{  dx }\OtherTok{\textless{}{-}} \FunctionTok{outer}\NormalTok{(x\_eval, x\_train, }\StringTok{"{-}"}\NormalTok{)}
...
\NormalTok{m\_ll }\OtherTok{\textless{}{-}} \FunctionTok{local\_linear}\NormalTok{(x\_grid, x, y, }\AttributeTok{h =} \FloatTok{0.05}\NormalTok{)}
...
\item
  ...
  models, using kernel weights \(w_i = K_h(\tilde x - x_i)\) to emphasize nearby
  observations.
\item
...
\tightlist
\item
  Cross-validation can be used to avoid overfitting the data when we choose parameters.
\item
  We have seen how to estimate \(h\) for kernel-based methods,
  \(k\) for \(k\)-NN regression estimates, and \(\lambda\) (or degrees of freedom)
...
\NormalTok{    T2 }\OtherTok{\textless{}{-}} \FunctionTok{colSums}\NormalTok{(x}\SpecialCharTok{*}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{dx)}\SpecialCharTok{*}\NormalTok{K)}

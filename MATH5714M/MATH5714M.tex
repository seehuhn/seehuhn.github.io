...
##  KDE1 1439.346 1455.869 1503.0398 1462.265 1470.506 6839.210  1000
##  KDE2  192.741  194.463  239.0309  195.037  195.980 4323.245  1000
##  KDE3  173.389  174.619  209.0022  175.193  176.095 2477.835  1000
\end{verbatim}
...
## Unit: microseconds
##     expr     min      lq     mean   median       uq      max neval
##  density 154.324 156.989 182.0305 164.2665 178.5345 6983.325  1000
\end{verbatim}
...
\tightlist
\item
  Suppose observations are denser to the left of a point \(x_0\) than to the
  right (i.e., \(f'(x_0) < 0\)), and the regression function has positive slope
  at \(x_0\). Explain geometrically why the Nadaraya-Watson estimator
  underestimates \(m(x_0)\).
\end{enumerate}

\begin{myanswers}
The Nadaraya-Watson estimator computes a weighted average
\(\hat m_h(x_0) = \sum_i w_i(x_0) y_i\), where the weights are determined by the
kernel centred at \(x_0\). When observations are denser to the left
(\(f'(x_0) < 0\)), more data points lie there and receive collectively greater
weight. Since \(m\) has positive slope, these left-side \(y\)-values are
systematically smaller than \(m(x_0)\). The weighted average is therefore pulled
...
\item
  ...
  Nadaraya-Watson bias at \(x = 0.9\) as a multiple of \(\mu_2(K) h^2\).
\end{enumerate}

\begin{myanswers}
We have \(m'(x) = 2\), \(m''(x) = 0\), \(f(0.9) = 0.2\), and \(f'(0.9) = -2\), giving
\(f'(0.9)/f(0.9) = -10\). The Nadaraya-Watson bias is
\begin{equation*}
  \mathop{\mathrm{bias}}\bigl(\hat m_h(0.9)\bigr)
  \approx \mu_2(K) \left( -10 \cdot 2 + \frac{1}{2} \cdot 0 \right) h^2
  = -20 \mu_2(K) h^2.
\end{equation*}
...
\tightlist
\item
  Compute the local linear bias at the same point. Explain geometrically why
